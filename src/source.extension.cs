// ------------------------------------------------------------------------------
// <auto-generated>
//     This file was generated by VSIX Synchronizer 1.0.45
//     Available from https://marketplace.visualstudio.com/items?itemName=MadsKristensen.VsixSynchronizer64
// </auto-generated>
// ------------------------------------------------------------------------------
namespace Modes
{
    internal sealed partial class Vsix
    {
        public const string Id = "Modes.428ec7ea-b605-49cd-a32e-415bfa035ff2";
        public const string Name = "Modes";
        public const string Description = @"One-click access to optimized Visual Studio configurations. Switch between Low Power, Focus, Performance, and Presenter modes instantly";
        public const string Language = "en-US";
        public const string Version = "1.0";
        public const string Author = "Mads Kristensen";
        public const string Tags = "zen, focus, performance, battery, power, presenter";
        public const bool IsPreview = false;
    }
}
